...
{
    public partial class SettingForm_No4 : Form
    {
        [DllImport("kernel32")]
        ...
        private Bench_4 m_ParentFormHandle;
        private BaseSetting_No4 m_BaseSettingHandle;
        private string strFilePath = Application.StartupPath + @"\4#\TestConfig.ini";//获取INI文件路径
        private string strSec = "TestConfig"; //INI文件名
        public SettingForm_No4(Form1 Handle, Bench_4 ParentForHandle)
        {
            InitializeComponent();
        ...
        private void SettingForm_No4_Load(object sender, EventArgs e)
        {
            Load_INI();
        ...
        private void Load_INI()
        {
            if (File.Exists(strFilePath))//读取时先要判读INI文件是否存在
            {
                ...
                TextBox_KeepPressure_1.Text = ContentValue(strSec, "KeepPressure_No1_Bench4");
                TextBox_KeepTime_1.Text = ContentValue(strSec, "KeepTime_No1_Bench4");
                string ButtonState = ContentValue(strSec, "KeepSelect_No1_Bench4");
                if (ButtonState == "OFF")
                {
                ...
                TextBox_KeepPressure_2.Text = ContentValue(strSec, "KeepPressure_No2_Bench4");
                TextBox_KeepTime_2.Text = ContentValue(strSec, "KeepTime_No2_Bench4");
                ButtonState = ContentValue(strSec, "KeepSelect_No2_Bench4");
                if (ButtonState == "OFF")
                {
                ...
                TextBox_KeepPressure_3.Text = ContentValue(strSec, "KeepPressure_No3_Bench4");
                TextBox_KeepTime_3.Text = ContentValue(strSec, "KeepTime_No3_Bench4");
                ButtonState = ContentValue(strSec, "KeepSelect_No3_Bench4");
                if (ButtonState == "OFF")
                {
                ...
                TextBox_Text_No.Text = ContentValue(strSec, "TestNo_Bench4");
        ...
        private void Button_SavePara_Click(object sender, EventArgs e)
        {
            ...
            if (KeepSelect1 == "OFF")
            {
                bool bSelect1 = false;
                bSelect1 = false;
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("ChanelSelect_No1_Bench4", bSelect1);
                if (code != 1)
                {
            ...
            {
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("KeepPressure_No1_Bench4", Press1);
                if (code != 1)
                {
                    //m_MainFrameHandle.m_LogWR.WriteLog(m_LogFilePath, "SetParamentForm");
                ...
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("KeepTime_No1_Bench4", Time1);
                if (code != 1)
                {
                    //m_MainFrameHandle.m_LogWR.WriteLog(m_LogFilePath, "SetParamentForm");
            ...
            if (KeepSelect2 == "OFF")
            {
                bool bSelect2 = false;
                bSelect2 = false;
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("ChanelSelect_No2_Bench4", bSelect2);
                if (code != 1)
                {
                    //m_MainFrameHandle.m_LogWR.WriteLog(m_LogFilePath, "SetParamentForm");
            ...
            {
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("KeepPressure_No2_Bench4", Press2);
                if (code != 1)
                {
                    //m_MainFrameHandle.m_LogWR.WriteLog(m_LogFilePath, "SetParamentForm");
                ...
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("KeepTime_No2_Bench4", Time2);
                if (code != 1)
                {
                    //m_MainFrameHandle.m_LogWR.WriteLog(m_LogFilePath, "SetParamentForm");
            ...
            if (KeepSelect3 == "OFF")
            {
                bool bSelect3 = false;
                bSelect3 = false;
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("ChanelSelect_No3_Bench4", bSelect3);
                if (code != 1)
                {
                    //m_MainFrameHandle.m_LogWR.WriteLog(m_LogFilePath, "SetParamentForm");
            ...
            {
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("KeepPressure_No3_Bench4", Press3);
                if (code != 1)
                {
                    //m_MainFrameHandle.m_LogWR.WriteLog(m_LogFilePath, "SetParamentForm");
                ...
                code = m_MainFrameHandle.m_PLCCommHandle.WriteData("KeepTime_No3_Bench4", Time3);
                if (code != 1)
                {
                    //m_MainFrameHandle.m_LogWR.WriteLog(m_LogFilePath, "SetParamentForm");
            ...
            WritePrivateProfileString(strSec, "KeepSelect_No1_Bench4", KeepSelect1, strFilePath);
            ...
            WritePrivateProfileString(strSec, "KeepSelect_No2_Bench4", KeepSelect2, strFilePath);
            ...
            WritePrivateProfileString(strSec, "KeepSelect_No3_Bench4", KeepSelect3, strFilePath);

            WritePrivateProfileString(strSec, "TestNo_Bench4", TextBox_Text_No.Text, strFilePath);
            //如果设置新的实验标号，则清除内存
        ...
        {
            m_BaseSettingHandle = new BaseSetting_No4(m_MainFrameHandle, this);
        ...
        private bool ReadPara()
        {
            ...
            int code = m_MainFrameHandle.m_PLCCommHandle.ReadData("KeepPressure_No1_Bench4", ref KeepPressure1, ref dt);
            if (code != 1)
            {
                return false;
            }

            code = m_MainFrameHandle.m_PLCCommHandle.ReadData("KeepPressure_No2_Bench4", ref KeepPressure2, ref dt);
            if (code != 1)
            {
                return false;
            }

            code = m_MainFrameHandle.m_PLCCommHandle.ReadData("KeepPressure_No3_Bench4", ref KeepPressure3, ref dt);
            if (code != 1)
            {
                return false;
            }

            code = m_MainFrameHandle.m_PLCCommHandle.ReadData("KeepTime_No1_Bench4", ref KeepTime1, ref dt);
            if (code != 1)
            {
                return false;
            }

            code = m_MainFrameHandle.m_PLCCommHandle.ReadData("KeepTime_No2_Bench4", ref KeepTime2, ref dt);
            if (code != 1)
            {
                return false;
            }

            code = m_MainFrameHandle.m_PLCCommHandle.ReadData("KeepTime_No3_Bench4", ref KeepTime3, ref dt);
            if (code != 1)
            {
                return false;
            }

            code = m_MainFrameHandle.m_PLCCommHandle.ReadData("ChanelSelect_No1_Bench4", ref ChanelSelect1, ref dt);
            if (code != 1)
            {
                return false;
            }

            code = m_MainFrameHandle.m_PLCCommHandle.ReadData("ChanelSelect_No2_Bench4", ref ChanelSelect2, ref dt);
            if (code != 1)
            {
                return false;
            }

            code = m_MainFrameHandle.m_PLCCommHandle.ReadData("ChanelSelect_No3_Bench4", ref ChanelSelect3, ref dt);
            if (code != 1)
            {
                return false;
            }
